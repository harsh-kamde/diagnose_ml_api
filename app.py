...
disease_specialist = {
    ...
    "AIDS": "Osteopathic",
    "Paroxysmal Positional Vertigo (vertigo)": "Otolaryngologist",
...
#==========================helper funtions================
def helper(dis):
    # Convert description to a single string
    desc_series = description[description['Disease'] == dis]['Description']
    desc = " ".join(desc_series.tolist())  # Convert Series to list and join into a string

    # Convert precautions to a list of lists
    ...
    med = med_series.tolist()  # Convert Series to list

    # Convert diet to a list
    die_series = diets[diets['Disease'] == dis]['Diet']
    die = die_series.tolist()  # Convert Series to list
...
@app.route("/")
def index():
    return "Welcome to medicine recommendation"

@app.route('/predict', methods=['POST'])
def home():
    data = request.get_json()  # Extract JSON data
    symptoms = data.get('symptoms')  # Retrieve 'symptoms' from JSON
    
    if not symptoms or symptoms.strip() == "":
        return jsonify({'message': 'Please enter valid symptoms (comma-separated)'}), 400

    user_symptoms = [s.strip() for s in symptoms.split(',')]
    predicted_disease = get_predicted_value(user_symptoms)

    if not predicted_disease:
        return jsonify({'message': 'No prediction could be made for the given symptoms'}), 404

    dis_des, pre, med, die, wrkout, specialist = helper(predicted_disease)

    response = {
        'predicted_disease': predicted_disease,
        'specialist': specialist,
        'dis_des': dis_des,
        'precautions': pre,  # Adjusted from 'my_precautions'
        'medications': med,
        'my_diet': die,
        'workout': wrkout
    }
